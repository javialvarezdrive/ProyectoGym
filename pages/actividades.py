...
def agendar_actividad():
    ...
    with st.form("form_agendar"):
        ...
        if monitores_df is not None and not monitores_df.empty:
            opciones_monitores = ["(Usar monitor logueado)"] + monitores_df.apply(
                lambda row: f"{row['nombre']} {row['apellidos']}", axis=1
            ).tolist()
            ...
            if monitor_sel == "(Usar monitor logueado)":
                ...
                monitor_id = fila["id"].values[0] if not fila.empty else st.session_state.user["id"]
        else:
            monitor_id = st.session_state.user["id"]
        ...
        if st.form_submit_button("Agendar actividad"):
            # Al agendar, no se asigna usuario (usuario_id = None)
...
def asignar_usuarios():
    ...
    # Se muestran todas las actividades en el rango
    df_acts = get_actividades(filtros)
    
    if not df_acts.empty:
         # Construir listado sin mostrar el ID; label compuesto por "Fecha - Turno - Actividad",
         # y añadir columna "Usuarios asignados" según si usuario_id es no nulo.
         datos = []
         ...
         # Crear un selectbox para elegir la actividad a asignar (la etiqueta muestra fecha, turno y actividad)
         mapping = {}
         opciones = []
         for _, row in df_acts.iterrows():
              fecha_str = format_date(row["fecha"])
